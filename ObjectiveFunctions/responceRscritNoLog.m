...
function rsCrit = responceRscritNoLog(peaks, voidTime, skew)
    ...
    if ~isempty(peaks)
        peaks = peaks(peaks(:,1)>(voidTime*skew),:);
    end
    ...
    if isempty(peaks)
        ...
        if numbOfPeaks == 1
            ...
            for row = 1:size(peakLabels,1)
                ...
                % 2*(Rt2-Rt1)/(W2+W1)
                rS(row) = (rt(peak2)-rt(peak1))./(0.5.*(width(peak2)+width(peak1)));
                %rS(row) = 1.177.*(rt(peak2)-rt(peak1))./(width(peak2)+width(peak1));
